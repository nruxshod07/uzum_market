...
export function createBlock(arr, place, titles) {
    let limit = 5
    let chekArrLength = arr.length - limit > 5 ? "Показать ещё 5" : `Показать ещё ${arr.length - limit}`
    ...
    show_more.onclick = () => {
        if (show_more.innerText === "Скрыть") {
            limit = 5
            goods_products(arr.slice(0, limit), box)
            show_more.innerHTML = arr.length - limit > 5 ? "Показать ещё 5" : `Показать ещё ${arr.length - limit}`
            return
