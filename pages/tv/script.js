...
let electriks_href = document.querySelector(".tv");
...
function sort() {
  ...
  title_sort_cont_h1.innerHTML = "ТВ";
  ...
  selected.onchange = () => {
    if (selected.value === popular_opt.value) {
      ...
      history.pushState(
        null,
        null,
        "/pages/kitchen/index.html" + `/?${moreExpensive_opt.value}`
      );
...
function reload(arr) {
  ...
  for (let item of arr) {
    ...
    let price_replace = item.price
      .toString()
      .replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, " ");

    title_p.innerHTML =
      item.title.split("").length > 45
    ...
    let loan_per_month = Math.round(item.price / 11);
    credit.innerHTML = ` ${loan_per_month} сум/мес`;
    ...
    poster_img_box_img.onclick = () => {
      location.href = "/pages/id/index.html?id=" + item.id;
